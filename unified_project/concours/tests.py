...
class ConcoursViewsTestCase(APITestCase):
    ...
    def test_published_concours_list(self):
        ...
        if 'results' in response.data:
            self.assertGreater(len(response.data['results']), 0, "La liste des concours est vide")
        else:
            self.assertGreater(len(response.data), 0, "Aucun concours trouvé")

    def test_category_list(self):
        url = reverse('concourscategory-list')
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertGreater(len(response.data['results']), 0, "La liste des catégories est vide")
    ...
    def test_subcategory_list(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertGreater(len(response.data['results']), 0, "La liste des sous-catégories est vide")
        self.assertEqual(response.data['results'][0]['name'], "Médecine")
    ...
    def test_search_view(self):
        """Test de la recherche dans les concours"""
        url = reverse('search') + '?q=concours'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        if 'results' in response.data:
            self.assertGreater(len(response.data['results']), 0, "Aucun concours trouvé dans la recherche")
        else:
            self.assertGreater(len(response.data), 0, "Aucun concours trouvé dans la recherche")
