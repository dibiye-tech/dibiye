...
const Searchbar = ({ onSearch }) => {
  ...
  return (
    <div className='relative container flex bg-white rounded-3xl h-12 p-1 md:h-16 shadow-sm items-center border-primary border-4'>
      <input
        type='text'
      ...
      {showDropdown && (
        <ul className="absolute w-full bg-white border border-gray-200 mt-1 z-10 top-full left-0 rounded-md shadow-lg" aria-live="polite">
          {history.length > 0 && (
  <>
    <li className="p-2 font-bold">Search History:</li>
        ...
        <li>
         <button onClick={handleClearHistory} className="p-2 w-full text-left text-red-500 hover:text-red-700">
          ...
          {searchTerm && history.length === 0 && suggestions.length === 0 && (
            <li className="p-2">No suggestions or history</li>
          )}
        </ul>
      )}
    </div>
